...
public class OpenClawGatewayWsClient extends WebSocketClient implements AutoCloseable {
    ...
    public void onOpen(ServerHandshake handshake) {
        ...
        challengeScheduler.schedule(() -> {
                    if (!connectFuture.isDone()) {
                        ...
                        if (nonce == null) {
                            debug("BASIC", "connect.challenge not received within {}ms, sending connect without nonce", CHALLENGE_TIMEOUT_MS);
                        }
        ...
        try {
            ...
            switch (type) {
                ...
                case "event":
                    ...
                    break;
                case "req":
                    debug("BODY", "Received unexpected req frame from Gateway: {}", root);
                    break;
                default:
    ...
    private void sendConnectHandshake() {
        ...
        try {
            if (connectFuture.isDone()) {
                debug("BASIC", "Skipping duplicate connect handshake");
                return;
            }
    ...
    String redactConnectHandshake(String json) {
        try {
            ...
            if (auth.isObject()) {
                tools.jackson.databind.node.ObjectNode sanitizedAuth =
                        (tools.jackson.databind.node.ObjectNode) sanitized.path("params").path("auth");
                for (String name : auth.propertyNames()) {
                    sanitizedAuth.put(name, "<redacted>");
                }
            }
            JsonNode device = root.path("params").path("device");
            if (device.isObject() && device.has("signature")) {
                ((tools.jackson.databind.node.ObjectNode) sanitized.path("params").path("device"))
    ...
    private CompletableFuture<HelloOk> beginConnectAttempt() {
        ...
        try {
            ...
            if (isOpen()) {
                debug("BASIC", "Closing existing WebSocket before reconnect");
                super.close();
            }
    ...
    void debug(String level, String message, Object... arguments) {
    ...
    private boolean handleConnectChallenge(JsonNode root) {
        ...
        try {
            ...
            if (nonce != null) {
                ...
                debug("BASIC", "Received connect.challenge with nonce, sending connect handshake");
            } else {
                debug("BASIC", "Received connect.challenge without nonce");
            }
